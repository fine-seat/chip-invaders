...
class CannonTester:
    ...
    def __init__(self, dut):
        ...
        _hex_path = Path(__file__).resolve().parent.parent / "src" / "rtl" / "single_barrel_cannon.hex"
    ...
    async def reset_module(self):
        ...
        self.pix_x.value = 0
        self.pix_y.value = 0
        await FallingEdge(self.v_sync)
        self.rst_n.value = 1
        await RisingEdge(self.v_sync)
...
async def test_reset(dut):
    ...
    clock = Clock(dut.v_sync, 10, unit="us")
    ...
    assert have == tester.single_barrel_cannon[0][0], f"test_reset (x): Expected {tester.single_barrel_cannon[0][0]}, got {have}"
...
async def test_sprite_graphics(dut):
    ...
    await RisingEdge(tester.v_sync)
